  ...
  const handleSaveDestination = async () => {
    try {
      ...
      const savedDestinationData = {
        // Retrait de user_id pour la démo
      ...
      if (error) {
        ...
        // Pour la démo, on continue même en cas d'erreur
        console.log("Continuing to planning page despite error (demo mode)");
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      try {
        ...
        // Étape 1 : Récupérer la dernière réponse au quiz (sans filtrer par user_id)
